...
export const useNeuroMatrix = () => {
  ...
  const isValidMove = useCallback((from: Position, to: Position, currentGrid: number[][]): boolean => {
    // 1. Check bounds
    ...
    const isStraight3 = (dRow === 3 && dCol === 0) || (dRow === 0 && dCol === 3);
  ...
  const getValidMoves = useCallback((pos: Position, currentGrid: number[][]): Position[] => {
    const moves: Position[] = [];
    const deltas = [
      // Straight 3
      { r: -3, c: 0 }, { r: 3, c: 0 }, { r: 0, c: -3 }, { r: 0, c: 3 },
      // Diagonal 2
